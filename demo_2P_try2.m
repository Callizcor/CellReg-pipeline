...
function params = setup_cellreg_parameters()
    % Simple parameter setup dialog for CellReg
    % Returns a structure with all parameters
    
    % Default values from your original code
    defaults.memory_efficient_run = 1;
    defaults.microns_per_pixel = 1.2;
    defaults.alignment_type = 'Translations and Rotations';
    defaults.use_parallel_processing = true;
    defaults.maximal_rotation = 30;
    defaults.transformation_smoothness = 2;
    defaults.reference_session_index = 1;
    defaults.sufficient_correlation_centroids = 0.2;
    defaults.sufficient_correlation_footprints = 0.3;
    defaults.maximal_distance = 12;
    defaults.p_same_certainty_threshold = 0.95;
    defaults.initial_registration_type = 'best_model_string';
    defaults.registration_approach = 'Probabilistic';
    defaults.model_type = 'best_model_string';
    defaults.p_same_threshold = 0.5;
    defaults.figures_visibility = 'on';
    ...
    prompt = {
        ...
        'Initial registration type (Spatial correlation/Centroid distance/best_model_string):'
        'Registration approach (Probabilistic/Simple threshold):'
        'Model type (Spatial correlation/Centroid distance/best_model_string):'
        'P_same threshold:'
        'Figures visibility (on/off):'
    };
    
    dlgtitle = 'CellReg Parameters Setup';
    dims = [1 60];
    
    definput = {
        num2str(defaults.memory_efficient_run)
        num2str(defaults.microns_per_pixel)
        defaults.alignment_type
        mat2str(defaults.use_parallel_processing)
        num2str(defaults.maximal_rotation)
        num2str(defaults.transformation_smoothness)
        num2str(defaults.reference_session_index)
        num2str(defaults.sufficient_correlation_centroids)
        num2str(defaults.sufficient_correlation_footprints)
        num2str(defaults.maximal_distance)
        num2str(defaults.p_same_certainty_threshold)
        defaults.initial_registration_type
        defaults.registration_approach
        defaults.model_type
        num2str(defaults.p_same_threshold)
        defaults.figures_visibility
    };
    
    answer = inputdlg(prompt, dlgtitle, dims, definput);
    
    if isempty(answer)
        error('Parameter setup cancelled');
    end
    
    % Parse answers
    params.memory_efficient_run = str2double(answer{1});
    params.microns_per_pixel = str2double(answer{2});
    params.alignment_type = answer{3};
    params.use_parallel_processing = str2num(answer{4});
    params.maximal_rotation = str2double(answer{5});
    params.transformation_smoothness = str2double(answer{6});
    params.reference_session_index = str2double(answer{7});
    params.sufficient_correlation_centroids = str2double(answer{8});
    params.sufficient_correlation_footprints = str2double(answer{9});
    params.maximal_distance = str2double(answer{10});
    params.p_same_certainty_threshold = str2double(answer{11});
    params.initial_registration_type = answer{12};
    params.registration_approach = answer{13};
    params.model_type = answer{14};
    params.p_same_threshold = str2double(answer{15});
    params.figures_visibility = answer{16};
end
...
%% Parameter Setup UI
params = setup_cellreg_parameters();

% Extract parameters
memory_efficient_run = params.memory_efficient_run;

%% Setting paths for the cell registration procedure:
% Define the root directory where CellReg is located
fileroot = 'C:\Users\Ido\Desktop\For_Ido\CellReg-master';

% USER INPUT: Choose how to select your files
% Option 1: Select specific files using GUI file picker
[file_names_only, data_directory] = uigetfile('*.mat', 'Select .mat files for cell registration', ...
    'C:\Users\Ido\Desktop\For_Ido\CellReg-master\SampleData\NoaDataFilter', 'MultiSelect', 'on');

if isequal(file_names_only, 0)
    error('No files selected. Script terminated.');
end

% Handle case where only one file is selected (uigetfile returns string instead of cell)
if ischar(file_names_only)
    file_names_only = {file_names_only};
end

% Create full file paths
number_of_sessions = length(file_names_only);
file_names = cell(1, number_of_sessions);
for it = 1:number_of_sessions
    file_names{it} = fullfile(data_directory, file_names_only{it});
end


% Defining the results_directory and creating the figures_directory:
results_directory = uigetdir('C:\Users\Ido\Desktop\For_Ido\CellReg-master\SampleData', 'Select directory to save the results');
figures_directory = fullfile(results_directory, 'Figures');
if exist(figures_directory,'dir') ~= 7
    mkdir(figures_directory);
end
figures_visibility = params.figures_visibility; % either 'on' or 'off' (in any case figures are saved)

% Display the files that will be processed
fprintf('Selected %d sessions for processing:\n', number_of_sessions);
for it = 1:number_of_sessions
    [~, filename, ext] = fileparts(file_names{it});
...
alignment_type = params.alignment_type; % either 'Translations', 'Translations and Rotations' or 'Non-rigid'
use_parallel_processing = params.use_parallel_processing; % either true or false
maximal_rotation = params.maximal_rotation; % in degrees - only relevant if 'Translations and Rotations' is used
transformation_smoothness = params.transformation_smoothness; % levels of non-rigid FOV transformation smoothness (range 0.5-3)
...
sufficient_correlation_centroids = params.sufficient_correlation_centroids; % smaller correlation imply no similarity between sessions
sufficient_correlation_footprints = params.sufficient_correlation_footprints; % smaller correlation imply no similarity between sessions
...
maximal_distance = params.maximal_distance; % cell-pairs that are more than 12 micrometers apart are assumed to be different cells
normalized_maximal_distance=maximal_distance/microns_per_pixel;
p_same_certainty_threshold = params.p_same_certainty_threshold; % certain cells are those with p_same>threshld or <1-threshold
...
initial_registration_type = params.initial_registration_type; % either 'Spatial correlation', 'Centroid distance', or 'best_model_string';
...
model_type = params.model_type; % either 'Spatial correlation' or 'Centroid distance'
p_same_threshold = params.p_same_threshold; % only relevant if probabilistic approach is used
...
comments = params.comments; % anything written here will be added to the log file
if strcmp(registration_approach,'Probabilistic')
    if strcmp(model_type,'Spatial correlation')
