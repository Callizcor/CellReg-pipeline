...
% MODIFIED: Now also returns roi_mapping structure that tracks original ROI indices
    ...
    IsCellProb_raw = fetch(IMG.IsCellProb & ...
                           sprintf('subject_id=%d AND session=%d', subject_id, session), ...
                           'probability');
    IsCellProb = [IsCellProb_raw.probability];
    
    % Fetch ROI data
    rois_raw = fetch(IMG.ROI & ...
                     sprintf('subject_id=%d AND session=%d', subject_id, session), ...
...
function create_roi_to_cellreg_lookup_tables(optimal_cell_to_index_map, roi_mappings, results_directory, session_numbers, subject_id, p_same_registered_pairs)
% CREATE_ROI_TO_CELLREG_LOOKUP_TABLES - Create structured lookup table
    ...
    if nargin < 6 || isempty(p_same_registered_pairs)
        fprintf('  P_same data: Not provided\n');
        has_p_same = false;
        p_same_registered_pairs = [];
    else
        fprintf('  P_same data: Provided as parameter\n');
        fprintf('    Class: %s\n', class(p_same_registered_pairs));
        fprintf('    Size: [%d x %d]\n', size(p_same_registered_pairs, 1), size(p_same_registered_pairs, 2));
        fprintf('    Is numeric: %d\n', isnumeric(p_same_registered_pairs));
        fprintf('    Is cell: %d\n', iscell(p_same_registered_pairs));
        
        % Handle different data types
        if iscell(p_same_registered_pairs)
            fprintf('    WARNING: p_same_registered_pairs is a cell array!\n');
            fprintf('    First cell class: %s\n', class(p_same_registered_pairs{1}));
            fprintf('    First cell size: [%s]\n', num2str(size(p_same_registered_pairs{1})));
            
            % If it's a cell array where each cell is a matrix, use it as-is
            if ~isempty(p_same_registered_pairs) && isnumeric(p_same_registered_pairs{1})
                fprintf('    Using as cell array of matrices (already in correct format)\n');
                has_p_same = true;
            else
                fprintf('    ERROR: Unexpected cell array format\n');
                has_p_same = false;
                p_same_registered_pairs = [];
            end
        elseif isnumeric(p_same_registered_pairs)
            fprintf('    P_same is numeric - will convert to matrices\n');
            has_p_same = true;
            
            % DIAGNOSTIC
            if size(p_same_registered_pairs, 1) >= 1
                fprintf('    Sample values (cell 1, first 5 pairs): %s\n', ...
                        num2str(p_same_registered_pairs(1, 1:min(5, size(p_same_registered_pairs, 2)))));
            end
        else
            fprintf('    ERROR: Unexpected data type for p_same_registered_pairs\n');
            has_p_same = false;
            p_same_registered_pairs = [];
        end
    end
    ...
    n_sessions = length(session_numbers);
    n_expected_pairs = n_sessions * (n_sessions - 1) / 2;
    fprintf('  Number of sessions: %d\n', n_sessions);
    fprintf('  Expected pairs per cell: %d\n', n_expected_pairs);
        ...
        for roi_idx = 1:n_rois_this_session
            ...
            if filtered_idx > 0
                ...
                if ~isempty(cellreg_id)
                    ...
                    if has_p_same
                        if iscell(p_same_registered_pairs)
                            % If already a cell array of matrices, just use it
                            if cellreg_id <= length(p_same_registered_pairs)
                                P_same_matrix_cell{current_row} = p_same_registered_pairs{cellreg_id};
                                if ~isempty(p_same_registered_pairs{cellreg_id})
                                    p_same_assigned_count = p_same_assigned_count + 1;
                                end
                            else
                                P_same_matrix_cell{current_row} = [];
                            end
                        elseif isnumeric(p_same_registered_pairs)
                            % Convert from pairs vector to matrix
                            if cellreg_id <= size(p_same_registered_pairs, 1)
                                p_mat = extract_p_same_matrix_from_pairs(...
                                    cellreg_id, p_same_registered_pairs, n_sessions);
                                P_same_matrix_cell{current_row} = p_mat;
                        ...
                        % DIAGNOSTIC: Print first few assignments
                        if p_same_assigned_count <= 3 && ~isempty(P_same_matrix_cell{current_row})
                            fprintf('    DEBUG: Assigned P_same to row %d (CellReg ID %d):\n', current_row, cellreg_id);
                            disp(P_same_matrix_cell{current_row});
                        end
                    else
                        P_same_matrix_cell{current_row} = [];
                    end
    ...
    fprintf('  DIAGNOSTIC: Total P_same matrices assigned: %d\n', p_same_assigned_count);
    
    % Create the final lookup table structure
    ROI_to_CellReg_Lookup = struct();
    ROI_to_CellReg_Lookup.CellReg_ID = CellReg_ID_array;
    ROI_to_CellReg_Lookup.ROI_number = ROI_number_array;
    ROI_to_CellReg_Lookup.Session = Session_array;
    ROI_to_CellReg_Lookup.P_same_matrix = P_same_matrix_cell;
    ...
    if has_p_same
        ...
        fprintf('    Non-empty P_same matrices: %d\n', p_same_assigned_count);
    else
...
% EXTRACT_P_SAME_MATRIX_FROM_PAIRS - Convert p_same_registered_pairs vector to matrix
    ...
    if isempty(p_same_registered_pairs) || cell_idx < 1 || cell_idx > size(p_same_registered_pairs, 1)
        p_same_matrix = [];
        return;
    end
    ...
    if isempty(pair_vector) || all(isnan(pair_vector))
        p_same_matrix = [];
        return;
    end
...
% RUN_CELLREG_PIPELINE - Execute the complete CellReg registration
   ...
   try
    ...
    if ~isempty(cells_multi_session)
        ...
        n_heatmaps = min(10, length(cells_multi_session));
        for i = 1:n_heatmaps
    ...
    if ~isempty(cells_all_sessions)
        ...
        n_zoomed = min(5, length(cells_all_sessions));
        for i = 1:n_zoomed
...
if isfield(params, 'roi_mappings') && ~isempty(params.roi_mappings)
    ...
    % Create ROI-to-CellReg lookup table with p_same_registered_pairs
    if strcmp(registration_approach, 'Probabilistic') && exist('p_same_registered_pairs', 'var')
        fprintf('  Passing p_same_registered_pairs to lookup table creation...\n');
        ...
        fprintf('  p_same_registered_pairs not available (non-probabilistic registration or variable not found)\n');
        create_roi_to_cellreg_lookup_tables(optimal_cell_to_index_map, ...
                                           params.roi_mappings, ...
                                           results_directory, ...
                                           params.session_numbers, ...
...
% PLOT_ZOOMED_CELL_COMPARISON - Three-row zoomed comparison with optimized rendering
% MODIFIED: Now shows actual ROI numbers from database and fixed P_same heatmap
%
% Row 1: Zoomed out (full FOV) with small cross at cell location
% Row 2: 3x zoom with thin contour around cell
% Row 3: 6x zoom with only center point
% Right: P_same heatmap
    ...
    % Calculate figure width (3 rows of sessions + heatmap)
    if has_p_same
        fig_width = 250*n_sessions_present + 400; % Extra space for heatmap column
    else
    ...
    fig = figure('Position', [50, 50, fig_width, 900], ...
                 'Name', fig_name, ...
    ...
    if has_p_same
        total_cols = n_sessions_present + 1; % Extra column for heatmap
    else
        total_cols = n_sessions_present;
    end
    
    % Plot each session in 3 rows
    for i = 1:n_sessions_present
        sess = sessions_present(i);
        ...
        if ~isempty(roi_mappings) && sess <= length(roi_mappings)
            ...
            if ~isempty(original_position)
                ...
                fprintf('    Session %d: CellReg filtered index %d → Database ROI #%d\n', ...
                        sess, cell_local_idx, actual_roi_number);
            else
        ...
        try
            ...
            % Show actual ROI number from database
            title(sprintf('S%d - 3x Zoom (ROI #%d)', actual_session_num, actual_roi_number), ...
                  'FontSize', 10, 'FontWeight', 'bold');
            axis on;
            hold off;
            ...
            % Show error in all three rows
            for row = 1:3
                subplot(3, total_cols, col_idx + (row-1)*total_cols);
    ...
    %% Add P_same heatmap on the right
    if has_p_same
        try
            p_same_matrix = extract_p_same_for_cell(cell_idx, optimal_cell_to_index_map, ...
                                                    p_same_registered_pairs, sessions_present);
            
            % FIXED: Create heatmap in the last column, spanning all 3 rows
            % Calculate correct subplot indices for the rightmost column
            heatmap_subplots = [total_cols, ...                    % Row 1, last column
                               total_cols + total_cols, ...        % Row 2, last column
                               total_cols + 2*total_cols];         % Row 3, last column
            
            subplot(3, total_cols, heatmap_subplots);
            ...
            xlabel('Session', 'FontSize', 11, 'FontWeight', 'bold');
            ylabel('Session', 'FontSize', 11, 'FontWeight', 'bold');
            title('P(Same Cell)', 'FontSize', 12, 'FontWeight', 'bold');
    ...
    sgtitle(sprintf('Cell %d - Multi-Scale Comparison (Sessions: %s)', cell_idx, actual_sessions_str), ...
            'FontSize', 14, 'FontWeight', 'bold');
    ...
    fprintf('  Created 3-level zoomed comparison for cell %d (sessions: %s)\n', ...
...
function plot_upset_style(presence_matrix, session_numbers)
    ...
    n_show = min(5, length(sorted_counts));
